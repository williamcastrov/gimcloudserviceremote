...
import { Modal, Button, Form, Input, Typography, Row, Col, Select, DatePicker, InputNumber } from "antd";
...
*/
  ...
  const [modalTiemposManual, setModalTiemposManual] = useState(false);
  ...
  const [tipooperacion, setTipoOperacion] = useState(0);setCodigoCombo
  ...
  const [marcaEquipo, setMarcaEquipo] = useState(0);
  ...
  const [observacionInterna, setObservacionInterna] = useState(0);
  ...
  const [fechaIniciaActividad, setFechaIniciaActividad] = useState(finaltransporte_cosv);
  const [fechaTerminaActividad, setFechaTerminaActividad] = useState(finaltransporte_cosv);
  ...
  const visualizarOT = async () => {
    window.open("https://service.gimcloud.co/api/ordenesserv/generarPdf/" + id_actividad, "PDF")
  }

  const pasarARevision = async () => {
    console.log("USUARIO : ", props.idUsu)
    ...
    if (!result.data && (props.idUsu != 1 && props.idUsu != 60 && props.idUsu != 3 && props.idUsu != 4 && props.idUsu != 26)){
      swal("Control Calificación OT", "La actividad no registra Calificación", "warning", { button: "Aceptar" });
      return
    }else
    if (props.idUsu != 1 || props.idUsu != 60 || props.idUsu != 3 || props.idUsu != 4 || props.idUsu != 26) {
...
/*
    if (!props.listActividadActiva.horometro_cosv && props.idUsu != 60) {
      swal("Horometro", "El valor del Horometro no puede ser CERO", "warning", { button: "Aceptar" });
      return
    }
*/
  
 /*   if (props.listActividadActiva.horometro_cosv === 0 && props.idUsu != 60) {
      swal("Horometro", "El valor del Horometro no puede ser CERO", "warning", { button: "Aceptar" });
      return
    } else {
      */
      console.log("PASAR A REVISION ACTIVIDAD", id_actividad);
      const rest = await cumplimientooservServices.pasararevision(id_actividad);
  ...
  const cerrarOrden = async () => {
    if (estado_cosv === 24 || estado_cosv === 27) {
      ...
      if (horometro_cosv === 0 && props.idUsu != 60) {
        swal("Horometro", "El valor del Horometro no puede ser CERO", "warning", { button: "Aceptar" });
      } else {
  ...
  const abrirModalTiemposManual = () => {
    setModalTiemposManual(true);
  }

  const cerrarModalTiemposManual = () => {
    setModalTiemposManual(false);
  }
  ...
  const grabarValorHorometro = async () => {
    ...
    console.log("DIFERENCIA MINUTOS TRANSPORTE : ", x," - ", y)
...
*/
    let transporte = {
      id_actividad: id_actividad,
      fecha: fechaIniciaTransporte
    }

    let inicia = {
      id_actividad: id_actividad,
      fecha: fechaIniciaActividad
    }

    let termina = {
      id_actividad: id_actividad,
      fecha: fechaTerminaActividad
    }

    const res = await cumplimientooservServices.actualizainiciatransporte(transporte);
    ...
    const rest = await cumplimientooservServices.actualizafinaltransporte(inicia);

    if (rest.success) {
      swal("Transporte", "Fecha Final Transporte!", "success", { button: "Aceptar" });
      cerrarModalTiemposManual();
      console.log(rest.message)
      setActualiza(true);
    } else {
      swal("Transporte", "Error Actualizando Fecha Final Transporte!", "error", { button: "Aceptar" });
  ...
  const grabarCumplimiento = async () => {
...

    {
      ...
      setCumplimientoSeleccionado([{
        ...
        voltajesalidasulfatacion: "0",
        amperajecomponente: "0",
        celdasreferenciacomponente: marcaEquipo,
        cofreseriecomponentes: numeroSerie,
        estadocomponentes: "0",
  ...
  const guardarCambiosCumplimiento = async () => {
...

    {
      ...
      setCumplimientoSeleccionado([{
        ...
        observacioninterna: cumplimientoSeleccionado.observacioninterna,
      ...
      <Form {...layout} >
        ...
        <Row justify="center"  >
          ...
          <Col lg={8}>
            ...
            >
               <Input
                name='cofreseriecomponentes'
                placeholder="Número de serie"
                style={{ width: 250 }}
                onChange={(e) => setNumeroSerie(e.target.value)}
                value={cumplimientoSeleccionado && cumplimientoSeleccionado.cofreseriecomponentes}
              ></Input>
            </Item>
          </Col>
          <Col lg={8}>
            ...
            >
               <Input
                name='celdasreferenciacomponente'
                placeholder="Marca del Equipo"
                style={{ width: 250 }}
                onChange={(e) => setMarcaEquipo(e.target.value)}
                value={cumplimientoSeleccionado && cumplimientoSeleccionado.celdasreferenciacomponente}
              ></Input>
            </Item>
          </Col>
        </Row>
          ...
          <Col lg={14}>
            ...
            >
              <Input
                ...
                //value={observacion}
                onChange={(e) => setObservacionInterna(e.target.value)}
                value={cumplimientoSeleccionado && cumplimientoSeleccionado.observacioninterna}
              ></Input>
            </Item>
          </Col>
        </Row>
  ...
  const onChangeTiempoTransporte = (value, dateString) => {
    //console.log('Selected Time: ', value);
    //console.log('Formatted Selected Time: ', dateString);
    setFechaIniciaTransporte(dateString);
  };

  const onChangeTiempoIniciaActividad = (value, dateString) => {
    setFechaIniciaActividad(dateString);
  };

  const onChangeTiempoFinalActividad = (value, dateString) => {
    setFechaTerminaActividad(dateString);
  };

  const grabarTiemposManual = (
    <div className="App">
      <Title align="center" type="warning" level={4} className="margensuperior">
        REGISTRO TIEMPOS ACTIVIDAD
      </Title>
      <Row justify="center" >
        <Col lg={16}>
          <Item
            label="Fecha Hora Transporte"
          >
            <DatePicker showTime
             
              name="fechaactividad"
              onChange={onChangeTiempoTransporte}
            ></DatePicker>
          </Item>
        </Col>
        <Col lg={16}>
          ...
          >
            <DatePicker showTime
              className="espaciodos"
              name="fechaactividad"
              onChange={onChangeTiempoIniciaActividad}
            ></DatePicker>
          </Item>
        </Col>
        <Col lg={16}>
          <Item
            className="espacios"
            label="Termina Actividad"
          >
            <DatePicker showTime
              className="espacio"
              name="fechaactividad"
              onChange={onChangeTiempoFinalActividad}
            ></DatePicker>
          </Item>
        </Col>
      </Row>
      ...
      </Title>
{
  ...
  //onClick={() => iniciarCumplimiento()}
}
      <div className="container form">
        <ListGroup className="listgroup" as="ul" >
          <ListGroup.Item as="li" active type="button" variant="secondary" >
            INICIA TRANSPORTE : {iniciatransporte_cosv}
          </ListGroup.Item>
          <ListGroup.Item as="li" type="button" onClick={() => abrirModalTiemposManual()} variant="secondary">
            REGISTRAR TIEMPOS MANUAL
          </ListGroup.Item>
          ...
          <ListGroup.Item as="li" type="button" onClick={() => visualizarOT()} variant="secondary">
            VISUALIZAR OT
          </ListGroup.Item>
        </ListGroup>
      </div>
      ...
      >
        <Images numeroactividad={id_actividad} operario={operario} modalFotos={modalFotos} setModalFotos={setModalFotos}  />
      </Modal>
      ...
      <Modal
        title="REGISTRAR TIEMPOS MANUAL" visible={modalTiemposManual}
        onOk={cerrarModalTiemposManual}
        width={600}
        closable={false}
        footer={[
          <Button type="primary" danger onClick={cerrarModalTiemposManual} > Regresar </Button>,
          <Button type="primary" onClick={iniciarTransporte} > Enviar </Button>
        ]}
      >
        {grabarTiemposManual}
      </Modal>
